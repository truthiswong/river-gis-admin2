  ...
  methods: {
    ...
    onWaterFlotage() {
      if (this.waterFlotage) {
        ...
        for (const item of this.drawPage) {
          if (item.drawType.name == '水面漂浮物') {
            if (item.locationType.code == 'point') {
              ...
              // }
              item.latlng = {}
              item.latlng.lat = item.point[1]
              item.latlng.lng = item.point[0]
              console.log(item)
              console.log(item.latlng)
              this.waterFlotagePoint.push(item)
              console.log(this.waterFlotagePoint)
            }
        ...
        let markerTool
        for (const item of this.waterFlotagePoint) {
          console.log(item.latlng, item.innerName, item.id)
          markerTool = new T.Marker(item.latlng, { title: item.innerName, id: item.id })
          this.map.addOverLay(markerTool)
    ...
    mapDrawPoint(latlng, index, id, code) {
      ...
      if (code == 'riskSource') {
        markerTool.addEventListener('click', this.sourceRiskClick)
      } else if (code == 'discharge') {
        markerTool.addEventListener('click', this.sourceRiskClick)
      } else if (code == 'floatage') {
        markerTool.addEventListener('click', this.floatageClick)
      }
    },
    //绘制线
